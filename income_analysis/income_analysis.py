...
def load_and_process_data():
    ...
    try:
        ...
        with open('/12241-0001_en.csv', 'r', encoding='utf-8-sig') as f:
            lines = f.readlines()
        ...
        # Debug: Show raw data (hidden for cleaner interface)
        # st.write("Raw data after manual parsing:")
        ...
        # Debug: Show what household types we found (hidden for cleaner interface)
        # st.write("Found household types in data:")
        # st.write(df['household_type_clean'].unique())
        ...
        # Debug: Print some info about the data (hidden for cleaner interface)
        # st.write("Processed data:")
        # st.write(f"Data shape: {result_data.shape}")
        # st.write(f"Columns: {result_data.columns.tolist()}")
        # st.write("Sample data:")
        # st.write(result_data.head())
        # if len(result_data) > 0:
        #     st.write("Unique years:")
        #     st.write(sorted(result_data['year'].unique()))
        #     st.write("Unique household types:")
        #     st.write(sorted(result_data['household_type_english'].unique()))
        # else:
        #     st.write("No data found after processing!")

        return result_data, df

    except Exception as e:
        st.error(f"Error loading data: {str(e)}")
        st.write("Please make sure the CSV file is in the correct location: /Users/akar/Downloads/12241-0001_en.csv")
        return None, None
